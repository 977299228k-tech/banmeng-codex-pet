...
- 持久保存饱食度、心情值、精力、亲密度和性格倾向；升级插件不会重置。
- 可直接喂食和陪玩，Codex 工作与完成任务也会影响她的状态。
- 会根据需求和性格自主选择找零食、休息、探索、玩耍、求陪伴或伸懒腰。
...
运行时只在 `127.0.0.1:47831` 监听本地事件，拒绝浏览器跨站请求，不读取或保存账户令牌。生活状态采用原子写入并保留可恢复备份。

调试接口：`GET /health` 查看进程健康，`GET /state` 查看已脱敏的任务、额度和账户套餐状态。
